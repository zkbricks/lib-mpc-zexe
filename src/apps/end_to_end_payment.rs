...
mod tests {
    ...
    fn test_payment() {
        ...
        let (pk, vk) = end_to_end_payment::local_setup();
        let local_proof = end_to_end_payment::local_prover(pk);
        let new_coin = end_to_end_payment::mpc_compute(&local_proof.spent_coins[0]);

        let proof = plonk_prove(
            &crs, 
            vec![local_proof.spent_coins[0].clone()].as_slice(), 
            vec![new_coin].as_slice(),
            end_to_end_payment::mpc_prover::<8>
        );

        end_to_end_payment::local_verifier(vk, &local_proof);
